...
std::atomic<bool> CAMERA::should_stop(false);

// Global variable to track detection state (similar to your sensor readings)
extern bool bbox_threshold_met;
extern float last_detection_area;
extern int last_detection_class;

static inline float sigmoid(float x) {
    return 1.0f / (1.0f + expf(-x));
}

static inline const char* get_class_name(int class_idx) {
...
static void draw_bbox(uint16_t *fb, int x1, int y1, int x2, int y2, uint16_t color)
{
...
static void parse_yolo_layer(const ncnn::Mat& out, int stride, int anchor_start,
...
static void nms(std::vector<Detection>& detections, float iou_thresh)
{
...
void CAMERA::camera_thread(size_t sampling_rate, 
                          ...
                          float conf_threshold)
{
    ...
    if (cam < 0) { 
        perror("camera_thread: open video0"); 
        return;
    }
    ...
    if (ioctl(cam, VIDIOC_S_FMT, &fmt) < 0) {
        perror("camera_thread: VIDIOC_S_FMT");
        close(cam);
        return;
    }
    ...
    printf("Camera thread: Loading YOLO-Fastest-1.1 model...\n");
    if (yolo.load_param("/home/beagle/Yolo-Fastest/sample/ncnn/model/yolo-fastest-1.1.param")) {
        fprintf(stderr, "Camera thread: Failed to load param\n");
        close(fb);
        munmap(map, buf.length);
        close(cam);
        return;
    ...
    if (yolo.load_model("/home/beagle/Yolo-Fastest/sample/ncnn/model/yolo-fastest-1.1.bin")) {
        fprintf(stderr, "Camera thread: Failed to load model\n");
        close(fb);
        munmap(map, buf.length);
        close(cam);
        return;
    ...
    printf("Camera thread: Streaming with YOLO-Fastest detection...\n");
    printf("Camera thread: Detecting cup, skateboard, mouse (threshold area: %.1f)\n", 
           bbox_area_threshold);
    ...
    while (!should_stop.load())
    {
        ...
        if (ioctl(cam, VIDIOC_QBUF, &buf) < 0) {
            perror("camera_thread: QBUF");
            break;
        }

        if (ioctl(cam, VIDIOC_DQBUF, &buf) < 0) {
            perror("camera_thread: DQBUF");
            break;
        }

        uint8_t *yuyv = (uint8_t*)map;

        // Convert camera to framebuffer
        ...
        if (frame_count % 3 == 0) {
            ...
            ex.extract("120_854", out_large);
            ex.extract("129_922", out_small);

            std::vector<Detection> all_detections;
            parse_yolo_layer(out_large, 32, 3, all_detections, conf_threshold);
            parse_yolo_layer(out_small, 16, 0, all_detections, conf_threshold);

            nms(all_detections, 0.45f);

            detections = all_detections;

            // Check if any detection meets the area threshold
            for (const auto& det : detections) {
                float bbox_area = det.w * det.h;
                
                if (bbox_area >= bbox_area_threshold) {
                    printf("Camera thread: THRESHOLD MET! %s detected with area %.1f (threshold: %.1f)\n",
                           get_class_name(det.class_id), bbox_area, bbox_area_threshold);
                    printf("  Confidence: %.2f, Position: [%.1f, %.1f], Size: [%.1f x %.1f]\n",
                           det.confidence, det.x, det.y, det.w, det.h);
                    
                    // Update global variables
                    bbox_threshold_met = true;
                    last_detection_area = bbox_area;
                    last_detection_class = det.class_id;
            ...
            if (!detections.empty() && !should_stop.load()) {
                printf("Camera thread frame %d: %zu detections\n", frame_count, detections.size());
                for (const auto& det : detections) {
                    printf("  - %s (%.2f) area=%.1f at [%.1f, %.1f]\n",
                           get_class_name(det.class_id), det.confidence,
                           det.w * det.h, det.x, det.y);
                }
            }
        } else {
        ...
        if (elapsed < (long)sampling_rate) {
            std::this_thread::sleep_for(std::chrono::milliseconds(sampling_rate - elapsed));
        }
    }

    // Clean shutdown
    ioctl(cam, VIDIOC_STREAMOFF, &buf.type);
    munmap(map, buf.length);
    close(cam);
    close(fb);
