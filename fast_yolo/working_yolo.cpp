...
// YOLO anchors for YOLO-Fastest (from the param file)
// -23304=12,1.200000e+01,1.800000e+01,3.700000e+01,4.900000e+01,5.200000e+01,1.320000e+02,1.150000e+02,7.300000e+01,1.190000e+02,1.990000e+02,2.420000e+02,2.380000e+02
                     ...
                     std::vector<Detection>& detections, float conf_thresh)
{
    ...
    // out is channels x height x width = 255 x grid_h x grid_w
    // Reshape to: grid_h x grid_w x 3 x 85
    for (int h = 0; h < grid_h; h++) {
        for (int w = 0; w < grid_w; w++) {
            for (int a = 0; a < 3; a++) {
                int base_channel = a * 85;
                
                const float* feat_ptr = out.channel(base_channel).row(h) + w;
                
                // Get values from the feature map
                ...
                if (!is_target_class(best_class)) continue;
...
int main()
{
    ...
    while (1)
    {
        ...
        if (frame_count % 3 == 0) {
            ...
            parse_yolo_layer(out_large, 32, 3, all_detections, 0.3f);  // stride=32, anchors 3-5
            parse_yolo_layer(out_small, 16, 0, all_detections, 0.3f);  // stride=16, anchors 0-2
        ...
        for (const auto& det : detections) {
            ...
            // Transform coordinates from model space (320x320) to camera space (160x120)
            cam_x = cam_x * CAM_W / MODEL_SIZE;
            cam_y = cam_y * CAM_H / MODEL_SIZE;
            cam_w = cam_w * CAM_W / MODEL_SIZE;
            cam_h = cam_h * CAM_H / MODEL_SIZE;

            // Then to framebuffer space with rotation
